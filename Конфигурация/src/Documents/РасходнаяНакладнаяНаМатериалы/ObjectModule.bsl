Процедура ОбработкаПроведения(Отказ, РежимПроведения)
    ...
    |ИЗ
    |    Документ.РасходнаяНакладнаяНаМатериалы.Материалы КАК РасходнаяНакладнаяМатериалы
    |ГДЕ
	...
	СписокПродукции = ТаблицаПродукции.ВыгрузитьКолонку("Материал");
    
    // Проверка на пустую ТЧ
    Если СписокПродукции.Количество() = 0 Тогда
        Отказ = Истина;
        Сообщить("Табличная часть Материалы пуста. Проведение отменено.");
        Возврат;
    КонецЕсли;
    ...
    Для Каждого СтрокаТЧ Из Материалы Цикл
        ...
        Для Каждого СтрокаОста Из ТаблицаОстатков Цикл
            Если СтрокаОста.Продукция = СтрокаТЧ.Материал Тогда // Ищем по ссылке
    ...
    Если Отказ Тогда
        Сообщить("Внимание! Отгрузка НЕ проведена из-за нехватки продукции. Комментарий: " + ТекстНехватки, СтатусСообщения.Важное);
    КонецЕсли;
